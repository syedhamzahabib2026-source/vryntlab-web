...
export function SiteFooter() {
  ...
  return (
    <footer className="mt-auto" style={{ background: "var(--surface)" }}>
      ...
      >
        {/* Top: logo + tagline — Merco footer-top-wrap */}
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <p className="max-w-[380px] text-[15px] font-normal leading-[1.55] text-[#8888a8]">
        ...
        <div className="flex flex-col gap-12 lg:flex-row lg:justify-between">
          ...
          {/* RIGHT: CTA — Merco "Let's create something impactful together." */}
          <div className="flex flex-col justify-between gap-8 lg:max-w-[440px]">
            <p className="text-[2rem] font-medium leading-[1.15] tracking-[-0.03em] text-[#F0F0FF] sm:text-[2.5rem] lg:text-[3rem]">
              Let&rsquo;s create something great together.
            </p>

            {/* Pill + circle arrow — exact Merco button pair */}
            <div className="flex items-center gap-3">
              <Link
                href="/#contact"
                className={`inline-flex items-center rounded-full bg-gradient-to-r from-[#7C3FFF] to-[#00E5FF] px-6 py-3 text-[16px] font-medium text-white shadow-[0_0_24px_-6px_rgba(124,63,255,0.45)] transition-[transform,box-shadow] duration-300 active:scale-[0.98] ${focusRing} [@media(hover:hover)]:hover:-translate-y-px [@media(hover:hover)]:hover:shadow-[0_0_36px_-4px_rgba(124,63,255,0.65)]`}
              >
                Say Hello
              </Link>
              <Link
                ...
                aria-hidden
                tabIndex={-1}
                className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-r from-[#7C3FFF] to-[#00E5FF] text-[18px] text-white shadow-[0_0_20px_-6px_rgba(124,63,255,0.4)] transition-transform duration-300 [@media(hover:hover)]:hover:-translate-y-px"
              >
                →
              </Link>
            </div>
        ...
        {/* Bottom bar: copyright + email + legal links */}
